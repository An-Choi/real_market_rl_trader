"""Generate readable feature-quality and trained-policy sensitivity reports."""
...
def main() -> None:
    ...
    boundaries = resolve_boundaries(
        meta=metadata, data_by_symbol=all_data, config=config
    )
    selected = {
    ...
    for symbol, frame in selected.items():
        environment = build_backtest_environment(frame, config)
        if agent is None:
    ...
    if len(pooled_observations) > args.max_samples:
        indices = np.linspace(
            0, len(pooled_observations) - 1, args.max_samples, dtype=int
        )
    ...
    sensitivity_by_feature = {row["feature"]: row for row in sensitivity}
    rows = [
        {
            **quality_row,
            **sensitivity_by_feature[quality_row["feature"]],
