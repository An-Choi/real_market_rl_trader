...
| 항목 | 결정 |
|------|------|
| 거래 단위 | 5분봉 execution, 1종목 × 연속 20거래일 에피소드, overnight 보유 허용 |
| Action | Discrete 6-action: 목표 주식 비중 `0/20/40/60/80/100%` |
| Position | 거래비용 차감 후 목표 비중으로 조정. 같은 목표 반복은 no-op 보유 |
| Observation | 5분 micro + 30분 context feature + portfolio state (일봉 regime은 ablation 후보) |
| Reward | 거래비용 차감 scaled log return. downside/drawdown/benchmark-relative는 ablation 후보 |
| 거래비용 | env transition에서 직접 차감: 수수료·스프레드·슬리피지 + 매도 증권거래세 0.20% |
| 종목 | 005930 1종목으로 baseline 완주. 코드는 처음부터 multi-symbol-ready |
| 정책 구조 | Single shared MaskablePPO policy. 6개 목표 비중 action은 모두 유효. HRL 사용 안 함 |
| 검증 | 학습 중 validation 전체 구간 최고 checkpoint 선택 + test 전체 평가 + baseline 비교 |
...
[Trading Environment]      env/src/env/trading_env.py (Gymnasium)
   ↓
[MaskablePPO Training]     experiments/train.py (SB3 Contrib)
   ↓
...
|   |   |-- env/          # trading_env.py — 목표 비중 기반 Gymnasium env
...
|-- agent/                # MaskablePPO, normalization, validation callback, baseline policies
