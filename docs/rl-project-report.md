...
> **현재 계약 변경:** 환경 action은 `Hold/Add/Clear`에서 목표 비중
> `0/20/40/60/80/100%`로 변경되었다. 이 문서의 기존 3-action artifact 성과는
> 당시 실험 기록이며 새 환경과 호환되지 않는다. 새 계약으로 재학습·재평가가 필요하다.
...
목표가 바뀔 때 거래비용 차감 후 포트폴리오의 주식 비중이 해당 목표가 되도록
매수·부분매도·전량매도를 수행한다. 같은 목표를 반복하면 no-op으로 보유하며,
모든 목표 action은 유효하다.
...
기존 Add/Clear 비대칭은 6단계 target-position action으로 해소했다. 새 action 계약은
기존 artifact와 호환되지 않으므로 동일 데이터에서 재학습한 뒤 성과를 다시 비교한다.
