...
**기준:** `FEATURE_SCHEMA_VERSION = 3` / artifact format v2
...
- 시간 grid: `n` = 당일 5분 bar 개수(**순수 정규장** grid — schema v3부터 종가
  동시호가는 관찰에서 제외), `t` = 당일 내 0-based step
- price source: 해당 5분 bar의 `Close` (정규장 체결가만 — 15:20 bar의 Close는
  15:19 마지막 정규 체결가이며 15:30 경매가가 아니다)
- `units_held_frac = selected_target_index / 5` (마지막으로 선택한 목표 비중 단계)
- `unrealized_pnl_norm = (held_market_value − cost_basis) / max(initial_cash, 1e-9)`
  ...
  - `cost_basis`는 실제 매수 notional의 누적값이다. 부분 축소 시 매도 주식 수
    비율만큼 함께 감소하며, 0% 목표로 청산하면 0이 된다.
...
- `tod_frac = min(당일 내 step / max(nominal_bars_per_day − 1, 1), 1.0)` (당일 기준,
  ...
  - `nominal_bars_per_day`는 artifact `env_params`에 기록된 **고정 상수**(config
    `nominal_bars_per_day`, 기본 64) — 그날 실제 데이터에 존재하는 bar 수(결손 시
    미래에 따라 달라지는 값)를 쓰지 않는다. 정상적으로 데이터 결손이 없는 날은
    실제 bar 수와 일치해 기존 값과 동일하지만, 결손일에도 이전 step들의 관측이
    바뀌지 않는다(causal).
  - 분모의 `max(·, 1)`은 `nominal_bars_per_day == 1`처럼 극단적인 설정에서도 코드와
    계약이 갈리지 않게 하기 위한 코드 그대로의 표기. 실제 bar 수가
    `nominal_bars_per_day`보다 많은 날은 `min(..., 1.0)`으로 clip된다.

계산에 필요한 파라미터(`unit_fraction`, `max_units`, `initial_cash`, `episode_days`,
`duration_horizon_bars`, `nominal_bars_per_day`)는 artifact의 `env_params`에
기록된다. 서빙 쪽은 **artifact의 값**을 사용한다 (config 재정의 금지).

## 2.5 체결 계약 (schema v3) — ExecPrice

관찰과 체결의 시간 역전을 금지하는 계약. feature 출력에는 관찰 컬럼 외에
**실행 전용** 컬럼 `ExecPrice`가 포함된다:

- 정의: 라벨 시각 `T` 이후 **같은 날 첫 정규 1분봉의 Open** (시장가 주문 근사).
  하루 마지막 결정(다음 정규 1분봉 없음)은 **15:30 종가 동시호가 단일가**.
  둘 다 없으면 NaN — env가 해당 bar `Close`로 fallback한다.
- `ExecPrice`는 **관찰이 아니다**: `FEATURE_COLUMNS`에 포함되지 않고 정규화
  대상도 아니며, observation 벡터에 절대 들어가지 않는다 (결정 시점 기준
  미래 가격이므로 넣으면 lookahead).
- env는 `ExecPrice` 컬럼이 없는 market_data를 **생성자에서 거부**한다
  (fail-closed). 관찰한 bar의 `Close`로 체결하는 것(same-bar fill)은 금지.
- valuation(`portfolio_value`, `held_market_value`)은 계속 `Close` 기준
  mark-to-market — 체결 현금흐름만 `ExecPrice`를 쓴다.
  `estimate_liquidation_cost`도 `ExecPrice` 기준.
...
| action | label | 의미 |
|---|---|---|
| 0 | `target_0pct` | 주식 목표 비중 0% |
| 1 | `target_20pct` | 주식 목표 비중 20% |
| 2 | `target_40pct` | 주식 목표 비중 40% |
| 3 | `target_60pct` | 주식 목표 비중 60% |
| 4 | `target_80pct` | 주식 목표 비중 80% |
| 5 | `target_100pct` | 주식 목표 비중 100% |

목표가 바뀌면 거래비용 차감 후 `held_market_value / portfolio_value`가 선택 비중이
되도록 주문 notional을 계산한다. 같은 목표를 반복하면 drift를 매 bar 재조정하지 않고
no-op으로 보유한다. 따라서 현재 선택된 목표 action이 Hold 역할을 한다. 여섯 action은
모두 유효하며 action mask는 전부 `True`다.
...
- 서버는 artifact의 `feature_schema_version`이 자신의 `FeatureEngineer` 버전과 다르면
  **기동 거부**.
- 서버는 artifact의 `action_space`가 기대 정의(discrete, n=6, labels 순서)와 다르면
  **기동 거부**.
...
```json
{
  ...
  "observation_dim": 13,
  "action_space": {"type": "discrete", "n": 6, "labels": ["target_0pct", "target_20pct",
    "target_40pct", "target_60pct", "target_80pct", "target_100pct"]},
...
meta = ArtifactMetadata(
    ...
    action_space={"type": "discrete", "n": 6,
                  "labels": ["target_0pct", "target_20pct", "target_40pct",
                             "target_60pct", "target_80pct", "target_100pct"]},
