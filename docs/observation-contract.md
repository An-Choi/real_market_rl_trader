...
**기준:** `FEATURE_SCHEMA_VERSION = 2` / artifact format v1
...
- `units_held_frac = units_held / max_units`
- `unrealized_pnl_norm = (held_market_value − cost_basis) / initial_cash`
...
- `holding_duration_norm = (t − entry_step) / max(n − 1, 1)` (미보유 시 0)
- `tod_frac = t / max(n − 1, 1)`
- 분모의 `max(·, 1)`은 1-bar day(결손/부분장/fixture)에서도 코드와 계약이
  갈리지 않게 하기 위한 코드 그대로의 표기

계산에 필요한 파라미터(`unit_fraction`, `max_units`, `initial_cash`)는 artifact의
`env_params`에 기록된다. 서빙 쪽은 **artifact의 값**을 사용한다 (config 재정의 금지).
...
**마감 강제청산:** 당일 마지막 bar에서는 env가 agent action을 무시하고 무조건 Clear한다.
서빙(Spring)도 동일 규칙을 적용해야 한다.
...
- `null` — 학습 시 observation 정규화 없음 (현행). 서버는 feature를 그대로 모델에 넣는다.
